...
{
  return this->getBaseName(points, valueSetNames) + ".spark";
}
...
) const
{
    ...
    //const wordList& tempNames(valueSets.size());   
    const List<Type>& values = *valueSets[0];
    //    this-> write(values,os);
    forAll(values, value)
    {
      Info()<<"Val = "<<values[value]<<nl;
    ...
    //Info()<<"Output valueset"<<nl<< valueSets[0]<<nl;
    //Info()<<"       columns"<<nl<< columns[0]<<nl;
}
...
{  
    Info()<<"Output valueset"<<nl<< valueSets<<nl;
}
